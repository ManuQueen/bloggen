<!-- We will need this ob_start() when we are redirecting, redirecting pieces of code in this application, since we are using a fxn called header() to redirect, thus Y we need to turn ob on by calling ob_start() fxn like this ob_start(); . NOTE: ob_start() this fxn is in charge of buffering our headers oe the script that way when we are done with the script it will send everything at the same time than to send ti separately since php is sending each request one by one. NOTE:Later on when we use header() in the body it will give an error since php has aleady send the header  -->
<?php ob_start(); ?> 
